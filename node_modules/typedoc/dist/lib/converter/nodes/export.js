...
let ExportDeclarationConverter = class ExportDeclarationConverter extends components_1.ConverterNodeComponent {
    ...
    convert(context, node) {
        var _a, _b;
        if (this.application.options.getValue('mode') === block_1.SourceFileMode.File) {
            return;
        }
        ...
        if (node.exportClause) {
            ...
            (_b = (_a = sourceFileSymbol) === null || _a === void 0 ? void 0 : _a.exports) === null || _b === void 0 ? void 0 : _b.forEach((symbol, key) => {
                if (key === 'default') {
                    return;
                }
                const source = context.checker.tryGetMemberInModuleExports(key.toString().replace(/^__/, '_'), thisModule);
                if (source) {
                    reference_1.createReferenceReflection(context, source, symbol);
                }
            });
        }
        return context.scope;
